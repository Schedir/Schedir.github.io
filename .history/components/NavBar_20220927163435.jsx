...
import React, {useState, useEffect, useHover} from 'react';

const NavBar = () => {
    const [nav, setNav] = useState(false);
    const [color, setColor] = useState('transparent')
    const [textColor, setTextColor] = useState('#081221')
    const [hoverColor, setHoverColor] = useState('#8AD2BA')
    ...
    useEffect(() => {
        const changeColor = () => {
            if(window.scrollY >= 850){
                ...
                setTextColor('#8AD2BA');
                setHoverColor('#081221');
            }
            else {
                setColor('transparent');
                setTextColor('#081221');
                setHoverColor('#8AD2BA')
            }
        };
    ...
    <div id='Navbar' style={{backgroundColor: `${color}`}} className='fixed left-0 top-0 w-screen z-10 ease-in duration-300'>
        <div className='max-w-[1500px] m-auto flex justify-between items-center p-4'>
            ...
            {/* <img src="/media/full_logo.png" className="mr-3 h-8" alt="Schedir Logo" /> */}
                <h1 href="/" style={{color: `${textColor}`, hoverStyle: `${hoverColor}`}} className=' {hoverStyle({hoverColor})} items-center font-bold text-3xl'>Schedir</h1>
            </a>
            ...
            <div class="hidden w-full md:block md:w-auto " id="navbar-default">
                <ul class="flex flex-col p-3 mt-auto bg-gray-50 rounded-lg border border-gray-100 md:flex-row md:space-x-8 md:mt-0 md:text-sm md:font-medium md:border-0 md:bg-gray-700/10">
                    ...
                    <li>
                    <a href="/logbook" style={{color: `${textColor}`}} class="block py-2 pr-4 pl-3 text-[16px] md:border-0 md:p-0">Logbook</a>
                    </li>
                    <li>
